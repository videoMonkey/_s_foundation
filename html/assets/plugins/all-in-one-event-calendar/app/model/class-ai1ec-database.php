...
class Ai1ec_Database
{
	...
	/**
	 * Check for given table existence
	 *
	 * @param string $table Name of table to check existence for
	 *
	 * @return bool True if table exists in database
	 */
	public function table_exists( $table ) {
		global $wpdb;
		$list       = $wpdb->get_col( 'SHOW TABLES' );
		$tables_map = array();
		foreach ( $list as $name ) {
			$tables_map[strtolower( $name )] = $name;
		}
		return ( isset( $tables_map[strtolower( $table )] ) );
	}
