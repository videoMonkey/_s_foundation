...
class Ai1ecFacebookConnectorPlugin extends Ai1ec_Connector_Plugin {
	...
	const FB_INFO_VARIABLE                  = '<label><a href="http://help.time.ly/customer/portal/articles/616553-how-do-i-use-the-facebook-import-" target="_blank">How do I use Facebook import?</a></label>';
	...
	private function install_schema() {
		...
		if (
			Ai1ec_Meta::get_option( self::FB_OPTION_DB_VERSION ) !=
			self::FB_DB_VERSION
		) {
			...
			if ( Ai1ec_Database::instance()->apply_delta( $sql ) ) {
				...
				trigger_error(
					'Failed to upgrade Facebook DB schema',
					E_USER_WARNING
				);
			}
	...
	private function install_cron() {
		...
		if (
			$token &&
			Ai1ec_Meta::get_option( self::FB_OPTION_CRON_VERSION ) !=
	...
	public function display_admin_notices(){
		...
		$message = Ai1ec_Meta::get_option( self::FB_OPTION_CRON_NOTICE, false );
		if ( $message === false ) {
			return;
		}
	...
	private function save_facebook_user_in_cache( Ai1ec_Facebook_Current_User $user ) {
		try {
			$this->persistance_context->write_data_to_persistence(
				serialize( $user )
			);
	...
	public function handle_feeds_page_post() {
		...
		$category        = isset( $_POST['ai1ec_facebook_feed_category'] )
			? $_POST['ai1ec_facebook_feed_category']
			: '';
